...
fact <- data.frame(read_excel("sample_info_142-153.xlsx"))
...
#Qualititative analysis
    ...
    ggplot(data = df) +
      ...
      labs(subtitle="Dependence of the number of detectable proteins on decline NA (HIT)")
    }
...
ggvenn(vennn2, 
  ...
  # write.table(C_spec, "C_spec_142-153.txt", append = FALSE, sep = " ", dec = ".", row.names = TRUE, col.names = TRUE)
  # write.table(I_spec, "I_spec_142-153.txt", append = FALSE, sep = " ", dec = ".", row.names = TRUE, col.names = TRUE)
  # write.table(K_spec, "K_spec_142-153.txt", append = FALSE, sep = " ", dec = ".", row.names = TRUE, col.names = TRUE)
  # write.table(M_spec, "M_spec_142-153.txt", append = FALSE, sep = " ", dec = ".", row.names = TRUE, col.names = TRUE)
...
boxplot(dat_knn, outline = FALSE, col = cols, main = "Raw data (HIT)")
...
boxplot(dat_log, outline = FALSE, col = cols, main = "Log-transformed data (HIT)")
  legend("topright", levels(fact$group_2), fill = pal, bty = "n", xpd = T)

#Normalization of data
  dat_norm <- normalizeQuantiles(dat_log) 
  head(dat_norm)
boxplot(dat_norm, col = cols, main = "Normalized data (HIT)")
  legend("topright", levels(fact$group_2), fill = pal, bty = "n", xpd = T)
...
#Limma - differentially expressed proteins
  ...
  # 
  # addWorksheet(Differ_proteins, "HITAEC I-C")
  # writeData(Differ_proteins, "HITAEC I-C", full_list_efit_IvsC2)
  # 
  # addWorksheet(Differ_proteins, "HITAEC C-K")
  # writeData(Differ_proteins, "HITAEC C-K", full_list_efit_CvsK2)
  # 
  # addWorksheet(Differ_proteins, "HITAEC C-M")
  # writeData(Differ_proteins, "HITAEC C-M", full_list_efit_CvsM2)
  # 
  # addWorksheet(Differ_proteins, "HITAEC I-K")
  # writeData(Differ_proteins, "HITAEC I-K", full_list_efit_IvsK2)
  # 
  # addWorksheet(Differ_proteins, "HITAEC I-M")
  # writeData(Differ_proteins, "HITAEC I-M", full_list_efit_IvsM2)
  # 
  # addWorksheet(Differ_proteins, "HITAEC M-K")
  # writeData(Differ_proteins, "HITAEC M-K", full_list_efit_MvsK2)
  # 
  # saveWorkbook(Differ_proteins, file = "HITAEC_diff_proteins.xlsx", overwrite = TRUE)
...
EnhancedVolcano(full_list_efit,
                ...
                ylim = c(0, 9),
                FCcutoff = 2,  
                title ="Vulcano all vs all (HIT)",
                labSize = 4.0,
                boxedLabels = F,
                colAlpha = 1)

EnhancedVolcano(full_list_efit_IvsC,
                lab = full_list_efit_IvsC$Gene.name, 
                x = 'logFC',
                y = 'adj.P.Val',
                pCutoff = 0.05,
                xlim = c(-5.5, 5.5), 
                ylim = c(0, 9),
                FCcutoff = 2,  
                title ="Vulcano I vs C (HIT)",
                labSize = 4.0,
                boxedLabels = F,
                colAlpha = 1)

EnhancedVolcano(full_list_efit_CvsK,
                lab = full_list_efit_CvsK$Gene.name, 
                x = 'logFC',
                y = 'adj.P.Val',
                pCutoff = 0.05,
                xlim = c(-5.5, 5.5), 
                ylim = c(0, 9),
                FCcutoff = 2,  
                title ="Vulcano C vs K (HIT)",
                labSize = 4.0,
                boxedLabels = F,
                colAlpha = 1)

EnhancedVolcano(full_list_efit_CvsM,
                lab = full_list_efit_CvsM$Gene.name, 
                x = 'logFC',
                y = 'adj.P.Val',
                pCutoff = 0.05,
                xlim = c(-5.5, 5.5), 
                ylim = c(0, 9),
                FCcutoff = 2,  
                title ="Vulcano C vs M (HIT)",
                labSize = 4.0,
                boxedLabels = F,
                colAlpha = 1)

EnhancedVolcano(full_list_efit_IvsK,
                lab = full_list_efit_IvsK$Gene.name, 
                x = 'logFC',
                y = 'adj.P.Val',
                pCutoff = 0.05,
                xlim = c(-5.5, 5.5), 
                ylim = c(0, 9),
                FCcutoff = 2,  
                title ="Vulcano I vs K (HIT)",
                labSize = 4.0,
                boxedLabels = F,
                colAlpha = 1)

EnhancedVolcano(full_list_efit_IvsM,
                lab = full_list_efit_IvsM$Gene.name, 
                x = 'logFC',
                y = 'adj.P.Val',
                pCutoff = 0.05,
                xlim = c(-5.5, 5.5), 
                ylim = c(0, 9),
                FCcutoff = 2,  
                title ="Vulcano I vs M (HIT)",
                labSize = 4.0,
                boxedLabels = F,
                colAlpha = 1)

EnhancedVolcano(full_list_efit_MvsK,
                lab = full_list_efit_MvsK$Gene.name, 
                x = 'logFC',
                y = 'adj.P.Val',
                pCutoff = 0.05,
                xlim = c(-5.5, 5.5), 
...
pheatmap(dat_norm[p_above_FC_top_for_heatmap,], annotation_col = fact[,c(2,1)], main = "Heatmap HIT C vs M")
  ...
  length(p_above_FC)
  p_above_FC_top_for_heatmap <- rownames(full_list_efit_IvsC)[(full_list_efit_IvsC$adj.P.Val <= 0.05)&((full_list_efit_IvsC$logFC >= 2)|(full_list_efit_IvsC$logFC <= -2))]
  length(p_above_FC_top_for_heatmap)
pheatmap(dat_norm[p_above_FC_top_for_heatmap,], annotation_col = fact[,c(2,1)], main = "Heatmap HIT I vs C")
  ...
  length(p_above_FC_top_for_heatmap)
pheatmap(dat_norm[p_above_FC_top_for_heatmap,], annotation_col = fact[,c(2,1)], main = "Heatmap HIT C vs K")
  ...
  length(p_above_FC_top_for_heatmap)
pheatmap(dat_norm[p_above_FC_top_for_heatmap,], annotation_col = fact[,c(2,1)], main = "Heatmap HIT I vs K")
  ...
  length(p_above_FC_top_for_heatmap)
pheatmap(dat_norm[p_above_FC_top_for_heatmap,], annotation_col = fact[,c(2,1)], main = "Heatmap HIT I vs M")
  ...
  length(p_above_FC_top_for_heatmap)
pheatmap(dat_norm[p_above_FC_top_for_heatmap,], annotation_col = fact[,c(2,1)], main = "Heatmap HIT M vs K")
